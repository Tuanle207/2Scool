...
import { DcpReportsService, RegulationsService, StudentsService, TaskAssignmentService } from '../common/api';
...
interface Props {
  ...
  updateDcpReport: ({dcpReportId}: {dcpReportId: string}) => void;
  initDcpReport: (payload: DcpReport.CreateUpdateDcpReportDto) => void;
}

const DCPReportUpdatePage: React.FC<Props> = ({
  dcpReport,
  sendingDcpReport,
  addClassToReport,
  ...
  React.useEffect(() => {
    const initData = async () =>  {
      ...
      initDcpReport(dcpReportRes as DcpReport.CreateUpdateDcpReportDto);
    };
  ...
  const handleSend = () => {
    ...
    ActionModal.show({
      ...
      onAccept: () => updateDcpReport({dcpReportId})
    });
  }
...
export default withRedux({
  ...
  dispatchProps: (dispatch: any) => ({
    ...
    initDcpReport: (payload: any) =>
      dispatch(DcpReportActions.initDcpReport(payload)),
    updateDcpReport: (payload: any) => 
      dispatch(DcpReportActions.updateDcpReport(payload)),
  })
});
